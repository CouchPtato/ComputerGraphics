...
// Scale and center vertices to make them visible within a viewport
void scaleAndCenter(float vertices[][3], int n, int viewportWidth, int viewportHeight)
{
    ...
    float scaleX = viewportWidth / (maxX - minX);
    float scaleY = viewportHeight / (maxY - minY);
    float scale = (scaleX < scaleY) ? scaleX : scaleY;

    float offsetX = (viewportWidth - (maxX - minX) * scale) / 2 - minX * scale;
    float offsetY = (viewportHeight - (maxY - minY) * scale) / 2 - minY * scale;

    for (int i = 0; i < n; i++) 
    {
        vertices[i][0] = vertices[i][0] * scale + offsetX + getmaxx() / 2 - viewportWidth / 2;
        vertices[i][1] = vertices[i][1] * scale + offsetY + getmaxy() / 2 - viewportHeight / 2;
    }
}
...
int main() 
{
    ...
    scaleAndCenter(vertices, n, getmaxx(), getmaxy());
